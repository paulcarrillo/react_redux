    ...
    value: function mapOnConstruction(error) {
      ...
      try {
        for (var _iterator = stackLines[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          var _stackLine$match = stackLine.match(/(.+)\(eval at (.+) \(.+?\), .+(\:[0-9]+\:[0-9]+)\)/),
              _stackLine$match2 = _slicedToArray(_stackLine$match, 4),
              atSomething = _stackLine$match2[1],
              file = _stackLine$match2[2],
              rowColumn = _stackLine$match2[3];
